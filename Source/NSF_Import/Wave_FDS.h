...
public:
	...
	BYTE		bWaveWrite;
	BYTE		bMain_On;
	int			nEdit_FreqAccum; // EDIT for storing sub-frequency average
	int			nEdit_FreqCount; // EDIT for storing sub-frequency average
	bool		bEdit_TableWrite; // EDIT for notifying writes to table
	...
	__forceinline void DoTicks(int ticks,BYTE mix)
	{
		...
		while(ticks)
		{
            ...
            nEdit_FreqAccum += subfreq; // edit store running average of mod freq
            nEdit_FreqCount++;
