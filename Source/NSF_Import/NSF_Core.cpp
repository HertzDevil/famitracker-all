...
void __fastcall CNSFCore::WriteMemory_pAPU(WORD a,BYTE v)
{
	...
	if(a <= 0x407F)
	{
		if(mWave_FDS.bWaveWrite)
        {
            ...
            mWave_FDS.bEdit_TableWrite = true; // EDIT
        }
	}
	else
	{
...
void CNSFCore::SetTrack(BYTE track)
{
	...
	mWave_FDS.nFreq.W = 0;
    mWave_FDS.nEdit_FreqAccum = 0; // EDIT
    mWave_FDS.nEdit_FreqCount = 0; // EDIT
    ...
    mWave_FDS.bEdit_TableWrite = true;
...
void CNSFCore::SetPlayCalls(float plays)
{
	...
	while(runto)
	{
		...
		{
			NSF_Import_Vsync(); // EDIT callback on sync
