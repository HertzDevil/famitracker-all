...
public:
	...
	bool		bEdit_LoopReset;
	bool		bEdit_DMCTriggered;
	...
	__forceinline int DoTicks(int ticks)		//returns number of burned cycles (burned by DMC's DMA)
	{
		...
		while(ticks)
		{
			...
			if(bDMCActive)
			{
				...
				{
					if (nDMCDMAAddr == nDMCDMAAddr_Load && !bEdit_LoopReset)
					{
						bEdit_DMCTriggered = true; // EDIT tracking DMC trigger
					}
					bEdit_LoopReset = false;
					...
					if(!nDMCBytesRemaining)
					{
						if(bDMCLoop)
						{
							...
							bEdit_LoopReset = true; // EDIT tracking DMC trigger
						}
