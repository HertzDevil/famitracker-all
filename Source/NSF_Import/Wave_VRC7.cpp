...
void CNSFCore::VRC7_Reset()
{
    ...
    {
        bEdit_VRC7_Trigger[i] = false;
        bEdit_VRC7_Release[i] = false;
    }
    bEdit_VRC7_Patch = true;
}
...
void CNSFCore::VRC7_Write(BYTE V)
{
	...
	switch(nVRC7Address >> 4)
	{
		...
		case 2:
			if(x>5) break;
            if ((V & 0x10) != (VRC7Chan[1][x] & 0x10)) // EDIT
            {
                bEdit_VRC7_Trigger[x] = (V & 0x10) != 0;
                bEdit_VRC7_Release[x] = (V & 0x10) == 0;
            }
			VRC7Chan[1][x] = V;
