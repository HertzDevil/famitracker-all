...
static Blip_Buffer	BlipBuffer;			// Mono
static Blip_Buffer	BlipBufferLeft;		// Stereo
static Blip_Buffer	BlipBufferRight;
...
bool CMixer::AllocateBuffer(unsigned int BufferLength, uint32 SampleRate, uint32 ClockRate)
{
	...
	BlipBufferLeft.sample_rate(SampleRate, BufferLength);
	BlipBufferLeft.clock_rate(ClockRate);

	BlipBufferRight.sample_rate(SampleRate, BufferLength);
	BlipBufferRight.clock_rate(ClockRate);
...
{
	BlipSynth.offset(Time, AmpLeft, &BlipBufferLeft);
	BlipSynth.offset(Time, AmpRight, &BlipBufferRight);
...
{
	uint32 SamplesRead, i = 0;
	//int32 ExtSample;

	if (Stereo) {
		BlipBuffer.clear();
		BlipBufferLeft.read_samples((blip_sample_t*)Buffer, Size, true);
		SamplesRead = (int)BlipBufferRight.read_samples((blip_sample_t*)(Buffer) + 1, Size, true);

		/*
		while (ExternalSamples > 0 && i++ < SamplesRead) {
			ExternalSamples--;
			((int32*)(Buffer))[i << 1] += ExternalSampleBuffer[i];
			((int32*)(Buffer))[(i << 1) + 1] += ExternalSampleBuffer[i];
		}
		*/
/*
		if (ExternalSamples > 0) {
			while (i++ < SamplesRead) {
				if (ExternalSamples > 0) {
					ExternalSamples--;
					ExtSample = ExternalSampleBuffer[i];
				}
				else
					ExtSample = 0xFFFF;

				((int32*)(Buffer))[i << 1] += ExtSample;
				((int32*)(Buffer))[(i << 1) + 1] += ExtSample;
			}
		}
*/		
		return SamplesRead;
	...
	SamplesRead = (int)BlipBuffer.read_samples((blip_sample_t*)Buffer, Size);
/*
	while (ExternalSamples > 0 && i++ < SamplesRead) {
		ExternalSamples--;
		((int32*)(Buffer))[i] += ExternalSampleBuffer[i];
	}
*/
	return SamplesRead;
}
...
int32 CMixer::GetChanOutput(uint8 Chan)
{
	...
	m_bVolRead = true;
	//if (Channels[Chan].VolMeasure > 0)
	//	Channels[Chan].VolMeasure--;
	return Ret;
}