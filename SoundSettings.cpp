...
void CSoundSettings::OnBnClickedOk()
{
	...
	switch (SampleRate->GetCurSel()) {
		case 0: theApp.m_iSampleRate = 11025; break;
		case 1: theApp.m_iSampleRate = 22050; break;
		case 2: theApp.m_iSampleRate = 44100; break;
		case 3: theApp.m_iSampleRate = 48000; break;
	}

	switch (SampleSize->GetCurSel()) {
		case 0: theApp.m_iSampleSize = 16; break;
		case 1: theApp.m_iSampleSize = 8; break;
	}

	//theApp.SetSettingBufferLength(Slider->GetPos());
	theApp.m_iBufferLength = Slider->GetPos();

	theApp.m_iBassFilter = ((CSliderCtrl*)GetDlgItem(IDC_BASS_FREQ))->GetPos();
	theApp.m_iTrebleFilter = ((CSliderCtrl*)GetDlgItem(IDC_TREBLE_FREQ))->GetPos();
	theApp.m_iTrebleDamping = ((CSliderCtrl*)GetDlgItem(IDC_TREBLE_DAMP))->GetPos();
...
BOOL CSoundSettings::OnInitDialog()
{
	...
	switch (theApp.m_iSampleSize) {
		case 16: SampleSize->SelectString(0, "16 bit"); break;
		case 8: SampleSize->SelectString(0, "8 bit"); break;
	}

	Slider->SetPos(theApp.m_iBufferLength);

	Text.Format("%i ms", theApp.m_iBufferLength);
	...
	BassSlider->SetPos(theApp.m_iBassFilter);
	TrebleSliderFreq->SetPos(theApp.m_iTrebleFilter);
	TrebleSliderDamping->SetPos(theApp.m_iTrebleDamping);
