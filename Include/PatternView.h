...
const int HEADER_HEIGHT = /*38*/36;		// Top header (channel names etc)
const int ROW_COL_WIDTH = /*30*/32;
...
struct stClipData2 {
	...
	//int StartCol;		// Used for single channel effect column manipulation
...
public:
	...
	void SetMeterValue(int Channel, int Value);
	...
	void Copy(stClipData2 *pClipData);
	void Cut();
	void Paste(stClipData2 *pClipData);
	void PasteMix(stClipData2 *pClipData);
	void Delete();
